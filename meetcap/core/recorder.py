    ...
    def start_dual_recording(
        ...
        try:
            ...
            self.session = RecordingSession(
                ...
                output_path=output_path,
                start_time=time.time(),
                device_name=f"BlackHole + Microphone (dual)",
                sample_rate=self.sample_rate,
                channels=self.channels,
            )
            ...
            if output_path.exists():
                output_path.unlink()
            raise RuntimeError(f"failed to start dual recording: {e}")
    
    def stop_recording(self, timeout: float = 5.0) -> Optional[Path]:
        """
        ...
        try:
            ...
            if output_path.exists() and output_path.stat().st_size > 44:  # wav header is 44 bytes
                ...
                console.print(
                    f"[green]✓[/green] recording saved: {output_path.name} "
